...
st.markdown("""
    ...
    """, unsafe_allow_html=True) # Changed from unsafe_allow_y_axis to html

# --- HEADER & LOGO ---
header_col1, header_col2 = st.columns([4, 1])
with header_col1:
    st.title("Mtrol Full-Cycle Analysis")
    st.write("### Upload Original Data (1s Synchronized Analysis)")
with header_col2:
...
def process_data(dev_upload, temp_upload):
    ...
    # Fill the 2-minute gaps with 1-second values
...
if device_file and temp_file:
    ...
    m_col1.metric("Device Identified", device_type)
    ...
    with st.expander("📁 View Synchronized Data Table"):
        st.dataframe(df, use_container_width=True)

else:
    st.info("👈 Upload both Device and Chamber CSV files in the sidebar to begin.")
